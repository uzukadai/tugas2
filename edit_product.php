...
$sql_edit = "SELECT * FROM product WHERE product_id = :product_id";

$stmt_edit = $koneksi->prepare($sql_edit);
$stmt_edit->bindParam(":product_id", $id);
$stmt_edit->execute();

$hasil = $stmt_edit->fetch();

if(isset($_POST['t_tambah'])){
    $sql = "UPDATE product SET product_name=:product_name, product_category_id=:product_category_id, list_price=:list_price, stock=:stock WHERE product_id=:product_id";
    ...
    <div class="col-lg-6 mt-5">
        <form action="" method="POST">
            <div class="form-group">
                ...
                <input value="<?php echo $hasil['product_name']; ?>" class="form-control" type="text" id="product_name" name="product_name">
            </div>        
            <div class="form-group">
                <label for="product_category">Kategori Produk</label>
                <input value="<?php echo $hasil['product_category_id']; ?>" class="form-control" type="number" id="product_category" name="product_category">
            </div>           
            <div class="form-group">
                <label for="list_price">Harga</label>
                <input value="<?php echo $hasil['list_price']; ?>" class="form-control" type="number" id="list_price" name="list_price">
            </div>           
            <div class="form-group">
                <label for="stock">Persediaan</label>
                <input value="<?php echo $hasil['stock']; ?>" class="form-control" type="number" id="stock" name="stock">
            </div>           
